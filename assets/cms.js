...
window.TextalkCMS = (function () {
  ...
  async function fromGitHub(collection) {
  ...
  async function fromManifest(collection) {
    try {
  ...
  async function listFiles(collection) {
    // Lokaal (dev): manifest eerst, zodat nog-niet-gepushte content zichtbaar is.
    // Productie: GitHub API eerst, zodat nieuwe /admin-content automatisch verschijnt.
    const h = location.hostname;
    const isLocal = location.protocol === "file:" ||
      /^(localhost|127\.|0\.0\.0\.0|\[?::1)/.test(h) || h === "";
    const order = isLocal
      ? [fromManifest, fromGitHub]
      : [fromGitHub, fromManifest];
    for (const src of order) {
      const names = await src(collection);
  ...
  return { list, get, esc, mdToHtml, showTile, newsTile, expertTile, fill };
})();
